...
`https://vyshak-harikumar.vercel.app` is the permanent site URL — no custom domain is planned. If that ever changes, set the `NEXT_PUBLIC_SITE_URL` environment variable — in Vercel's project settings (Production environment) for the deployed site, or in a local `.env.local` for testing:

```
NEXT_PUBLIC_SITE_URL=https://your-new-domain.com
```

`lib/site.ts` falls back to the Vercel URL if the variable isn't set. **No code changes are needed** — every metadata file, the sitemap, robots.txt, and the JSON-LD all read `SITE_URL` from `lib/site.ts`.
...
1. Go to [Google Search Console](https://search.google.com/search-console) and add your property using the Vercel URL (`https://vyshak-harikumar.vercel.app`) — that's the permanent site URL.
...
5. Google will fetch `https://vyshak-harikumar.vercel.app/sitemap.xml` — the entries come from `app/sitemap.ts`.
...
- `metadataBase` previously pointed at a custom domain that was never purchased and isn't live — this was a latent bug (canonical/OG URLs would have pointed at a dead domain). Fixed by defaulting to the Vercel URL, which is now the permanent site URL, switchable in one place via `NEXT_PUBLIC_SITE_URL` per the section above if that ever changes.
